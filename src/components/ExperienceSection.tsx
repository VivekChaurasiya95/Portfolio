...
type ExperienceProof = {
  title?: string;
  type?: 'Offer Letter' | 'Completion Certificate' | 'Badge' | 'Recognition' | 'Document';
  image: string;
  description?: string;
};
...
const normalizeProof = (proof: ExperienceProofInput, index: number): Required<ExperienceProof> => {
  ...
  return {
    ...
    description: proof.description || 'Supporting document/image for this experience.',
  };
};
...
const experiences = [
  ...
  },
  {
    ...
    location: 'Remote',
    duration: 'May 2026 - Present',
    description: 'Selected as an Open Source and AI/Agents contributor, collaborating with distributed teams on real-world repositories.',
    detailSummary: 'Contributing to OSS issues, fixes, and collaboration workflows in the AI/Agents track with emphasis on practical impact.',
    achievements: [
    ...
    proofs: [
      {
        ...
        description: 'Proof of contributor selection in the open-source program.',
      },
      {
      ...
      },
      {
        ...
        description: 'Certificate of successful completion of the open-source program.',
      },
    ],
  ...
  },
  {
    ...
    description: 'Active contributor in a community coding program, delivering feature enhancements and practical fixes in collaborative repositories.',
    detailSummary: 'Focused on community-driven development, feature refinement, and peer-reviewed improvements in active repositories.',
    achievements: [
    ...
    proofs: [
      ...
      },
      {
        ...
        description: 'Contributor badge for active issue resolution and features.',
      },
      {
        title: 'Completion Certificate',
        type: 'Completion Certificate',
  ...
  },
  {
    ...
    description: 'Completed McKinsey Forward to strengthen problem solving, communication, and leadership.',
    detailSummary: 'A structured professional development experience covering strategic thinking, communication, and leadership capabilities.',
    achievements: [
...
const ExperienceSection = () => {
  ...
  const selectedProofs = selectedExperience
    ? (selectedExperience.proofs ?? []).map((proof, idx) => normalizeProof(proof, idx))
    : [];
              ...
              <div className="flex-1 relative flex items-center justify-center p-4 md:p-8">
                <AnimatePresence mode="wait">
                  <motion.img
                    ...
                    src={selectedProofs[selectedProofIndex]?.image || defaultProofPreview}
                ...
                <div className="absolute bottom-4 left-4 right-4 md:bottom-8 md:left-8 md:right-8 bg-background/80 backdrop-blur-xl border border-border/50 rounded-2xl p-4 transform translate-y-2 opacity-0 group-hover/modal:translate-y-0 group-hover/modal:opacity-100 transition-all duration-500 shadow-xl z-30">
                  <h4 className="text-foreground font-semibold flex items-center gap-2">
                    ...
                    {selectedProofs[selectedProofIndex]?.title || 'Document Preview'}
                  </h4>
                  <p className="text-muted-foreground text-sm mt-1">
                    {selectedProofs[selectedProofIndex]?.description || 'Supporting document for this experience.'}
                  </p>
                </div>
              </div>
                  ...
                  {selectedProofs.map((proof, idx) => (
                    ...
                    >
                      ...
                      {selectedProofIndex === idx && (
                        <motion.div layoutId="active-thumbnail" className="absolute inset-0 bg-primary/20 backdrop-blur-[1px]" />
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-primary font-medium tracking-wide">
                    {Array.isArray(selectedExperience.role) ? selectedExperience.role.join(' • ') : selectedExperience.role}
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  {selectedExperience.achievements.map((achievement, i) => (
                    ...
                    >
                      ...
                      <p className="text-foreground/90 text-sm leading-relaxed">{achievement}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
